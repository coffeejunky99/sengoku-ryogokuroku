...
Git管理上の正本は本Markdown版である。DOCX版は人間向けの配布・レビュー用スナップショットとし、差異がある場合はMarkdown版を優先する。Phase02の正式マップデータは`docs/GDD/縦切り試作データ一覧_v0.1.md`を参照する。
...
| TimeScale      | 0 / 1 / 2 / 4  | 時間停止と進行倍率           |
| AutoPauseReason | 7種の列挙値   | 重要通知による自動停止理由   |
...
- `year`は正の整数とする。
- `month`は1以上12以下の整数とする。
- `day`は一般的な西暦の月日・閏年規則で有効な整数とする。
- 歴史的な旧暦は使用しない。
- JavaScript `Date`および端末timezoneへ依存しない。
- 検証と加算はDomainの純粋関数で行う。
- 全国マップUIでは`YYYY/MM/DD`形式で表示する。

時間進行では次の正式値を使用する。

| **項目** | **値** |
|----------|--------|
| `SIMULATION_STEP_MS` | 250 |
| `SIMULATION_TICKS_PER_GAME_DAY` | 4 |
| `MAX_SIMULATION_TICKS_PER_FRAME` | 8 |
| 1倍速 | 実時間1秒でゲーム内1日 |
| 2倍速 | 実時間1秒でゲーム内2日 |
| 4倍速 | 実時間1秒でゲーム内4日 |
| 0倍速 | simulation tickを生成しない |
...
縦切り試作シナリオの`startDate`は`{ year: 1561, month: 9, day: 1 }`（1561-09-01）とし、初期`timeScale`は`0`とする。

# 7. 大名家データ

## 7.1 Phase02表示用勢力データ

Phase02では完全な`ClanDefinition`を要求せず、全国マップ表示に必要な次の定義だけを使用する。
...
正式画像素材はPhase02の必須条件としない。素材が未配置の場合は`identificationSymbol`の文字または記号を色と併用する。
...
Phase02では完全な`CastleDefinition`や`CastleState`を要求せず、次の定義だけを使用する。
...
人口、農業、商業、防御、施設、城主、駐在武将などはPhase02表示用定義に含めない。
...
Phase02の街道は全て双方向とし、始点と終点を直線で描画する。`distance`は縦切り試作用の論理距離であり、移動時間や補給計算を確定する値ではない。`movementCostMultiplier`はPhase07で正式に決定し、Phase02表示用定義には含めない。
...
Phase02の座標は端末解像度から独立した論理座標とする。正式値と有効範囲は`docs/GDD/縦切り試作データ一覧_v0.1.md`を参照する。
...
Phase03では`currentDate`と`timeScale`を権威的なゲーム状態として扱う。`timeScale`の初期値は`0`とする。

`RngState`は将来のGameStateと保存データに必要な概念として維持するが、Phase03では具体型、RNGアルゴリズム、乱数生成処理を定義しない。RNGを最初に使用するPhaseでGDDおよび設計書を更新してから正式導入する。

次の値はfixed-step runnerだけが保持するruntime状態であり、GameStateへ含めない。
...
documentが非表示になった場合は`timeScale`を`0`へ変更し、runnerのruntime状態を破棄する。background滞在時間は`currentDate`へ加算せず、foreground復帰後も`timeScale`は`0`を維持する。この安全動作はUserSettingsで無効化できない。
...
将来の保存対象となる時間関連の権威的状態は`currentDate`、`timeScale`、UserSettingsの自動停止設定とする。runtime状態は保存しない。

Phase03ではSaveRepository、IndexedDB、SQLite、schema migration、セーブデータ復旧を実装しない。ロード直後の時間速度に関する保存・復元仕様は、セーブ機能を扱うPhaseで正式に決定する。
...
AutoPauseReasonとAutoPauseSettingsの対応は次のとおりとする。

自動停止7条件の初期値はすべて`true`とする。

| **AutoPauseReason** | **設定キー** | **表示上の意味** | **初期値** |
|---------------------|--------------|------------------|------------|
| `declaration_of_war` | `declarationOfWar` | 宣戦布告 | `true` |
| `enemy_entry` | `enemyEntry` | 敵軍の領内侵入 | `true` |
| `battle_start` | `battleStart` | 合戦開始 | `true` |
| `siege_start` | `siegeStart` | 城の包囲 | `true` |
| `important_death` | `importantDeath` | 大名または重要武将の死亡 | `true` |
| `historical_event` | `historicalEvent` | 歴史イベント | `true` |
| `low_treasury` | `lowTreasury` | 資金が一定値未満 | `true` |

有効な理由による停止要求は`timeScale`を`0`へ変更し、無効な理由では変更しない。通知を閉じても以前の速度へ自動復帰しない。

プレイヤーの意思決定による停止はAutoPauseReasonへ列挙値を追加せず、Application層の共通停止要求契約として別に扱う。各画面は時間状態を直接変更しない。内政、外交、軍事、武将、選択肢付きイベント、確認画面からの実際の呼び出しは各後続Phaseで実装する。

バックグラウンド停止はUserSettingsで無効化できない安全動作とする。
...
- 城の人口、農業、商業、防御、施設などPhase04以降で使用する初期値

- 街道の移動時間、移動補正、補給計算および`movementCostMultiplier`
...
能力値はGDDで具体値が定義されていないため、サンプルでは0を使用する。本番値ではない。
